...
class DisposisiController extends Controller
{
    ...
    public function inbox()
    {
        ...
        if ($userRole === 'kabag') {
            ...
            // Kasubag melihat:
            // 1. SEMUA disposisi berstatus 2 (menunggu verifikasi kasubag) — termasuk dari kabag langsung ke staf
            // 2. Disposisi yang dia kirim sendiri (untuk monitoring), semua status kecuali Selesai
            $disposisi_verifikasi = Disposisi::with(['surat', 'pengirim', 'penerima'])
                                ->where(function($q) use ($userId) {
                                    $q->where('status', DisposisiStatus::MenungguVerifikasiKasubag->value)
                                      ->orWhere(function($q2) use ($userId) {
                                          $q2->where('dari_user_id', $userId)
    ...
    public function selesai(LaporkanHasilKerjaRequest $request, $id)
    {
        ...
        // Alur verifikasi SELALU staf → kasubag → kabag, terlepas dari siapa pengirim disposisi
        $dataUpdate = [
        ...
        return back()->with('success', 'Laporan pekerjaan berhasil dikirim ke Kasubag untuk diverifikasi!');
    }
